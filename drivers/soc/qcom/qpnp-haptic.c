...
static int ze554kl_vmax_array[] = {
	464, 928, 1276, 1624, 2088, 2552, 2552
};
...
static int qpnp_hap_lra_auto_res_config(struct qpnp_hap *hap)
{
	...
	printk("[haptic] lra_res_cal_period = %d\n", hap->lra_res_cal_period);
	...
	if (hap->pmic_subtype == PM660_SUBTYPE) {
		...
		if (hap->auto_res_mode == QPNP_HAP_PM660_AUTO_RES_QWD) {
			printk("[haptic] PM660_AUTO_RES_QWD = True\n");
			hap->lra_res_cal_period = 0;
			printk("[haptic] And change qcom,lra-res-cal-period= 0\n");
	...
	printk("[haptic] write LRA_AUTO_RES_REG(0x4F) value = 0x%x\n", val);
	rc = qpnp_hap_masked_write_reg(hap,
...
static struct device_attribute qpnp_hap_attrs[] = {
	...
	__ATTR(vmax_level, (S_IRUGO | S_IWUSR | S_IWGRP),
                        NULL, vmax_level_store),
	__ATTR(vmax, 0664, vmax_show,
		vmax_store),
	__ATTR(shape, 0664, shape_show,
		shape_store),
};
...
static int qpnp_hap_config(struct qpnp_hap *hap)
{
	...
	if ((hap->act_type == QPNP_HAP_LRA) && hap->correct_lra_drive_freq
		...
		printk("[haptic]TRIM register = 0x%x\n", hap->clk_trim_error_code);
	...
	printk("[haptic] BRAKE_REG(0x48) = 0x%x\n", val);
	...
	if (hap->act_type == QPNP_HAP_LRA) {
		...
		if (!rc) {
			if (hap->pmic_subtype == PM660_SUBTYPE) {
				...
				printk("[haptic] pmic_subtype == PM660_SUBTYPE (0x1b)\n");
				...
				printk("[haptic] pmic_subtype != PM660_SUBTYPE\n");
		...
		*/
		...
		printk("[haptic] lra-qwd-drive-duration = 0x%x\n", hap->lra_qwd_drive_duration);

		hap->calibrate_at_eop = -EINVAL;
		rc = of_property_read_u32(pdev->dev.of_node,
			"qcom,lra-calibrate-at-eop", &hap->calibrate_at_eop);

		printk("[haptic] qcom,lra-calibrate-at-eop = %d\n", hap->calibrate_at_eop);
