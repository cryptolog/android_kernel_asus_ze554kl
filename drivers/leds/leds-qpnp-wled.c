...
static s64 wled_ovp_timestamp;
static int wled_ovp_count;
...
static irqreturn_t qpnp_wled_ovp_irq_handler(int irq, void *_wled)
{
	...
	if (g_asus_lcdID != AQU_LCD_TM)
		goto irq_return;

	if (wled_ovp_count <= 0) {
		wled_ovp_timestamp = ktime_to_ms(ktime_get()) + 3000;
		wled_ovp_count++;
	} else if (wled_ovp_count >= 9) {
		rc = qpnp_wled_write_reg(wled, QPNP_WLED_CURR_SINK_REG(wled->sink_base), 0x00);
		if (rc < 0)
			pr_err("[Display] Error in disabling all current sink rc=%d\n", rc);
		else
			printk("[Display] Disable all current sink due to WLED OVP detection\n");
	} else {
		time = ktime_to_ms(ktime_get());
		if (time < wled_ovp_timestamp) {
			wled_ovp_count++;
		} else{
			wled_ovp_timestamp = time + 3000;
			wled_ovp_count = 1;
		}
	}
